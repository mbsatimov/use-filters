...
export interface ExampleMeta {
  ...
  /** Source files shown in the Code tab, in order. Paths are relative to `components/examples`. */
...
export const examples: ExampleMeta[] = [
  {
    ...
    files: [
      { name: 'marketplace.tsx', path: 'marketplace/marketplace.tsx' },
      { name: 'products.ts', path: 'marketplace/products.ts' }
    ]
  },
  {
    slug: 'data-table',
    ...
    Component: DataTable,
    files: [
      { name: 'data-table.tsx', path: 'data-table/data-table.tsx' },
      { name: 'orders.ts', path: 'data-table/orders.ts' }
    ]
  }
];

...
<Callout>
  The old `FilterOption.icon` / `leftSlot` / `rightSlot` and the per-filter
  `className` are **deprecated** — rendering concerns don't belong on a headless
  core. Declare what you need on `FilterOptionMeta` / `FilterMeta` and pass it via
  `meta` (same data, typed by you). They keep working until 1.0.
</Callout>
